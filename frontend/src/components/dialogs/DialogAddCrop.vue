    ...
    <v-dialog v-model="props.model" persistent width="800px" min-height="100px">
      <v-card :loading="loading">
          ...
          <v-row>
            <v-col cols="11">
              <v-row>
                ...
                <v-col cols="11">
                  <span :style="{ color }">{{ loading ? 'Adicionando Cultura' : 'Adicionar Cultura' }}</span>
                </v-col>
              </v-row>
            </v-col>
            <v-col cols="1">
        ...
        <v-card-subtitle class="mb-3">
          ...
          <span v-else>Defina uma nova cultura</span>
        </v-card-subtitle>
        <v-divider />
        <v-card-text>
          <v-form ref="form">
            <v-row>
              <v-col cols="12">
                <v-text-field :disabled="loading" v-model="item.name" label="Nome" clearable :color="color"
        ...
        <v-card-actions>
          ...
          <v-btn :loading="loading" :disabled="loading" :color="color" @click="addCrop">
            Confirmar
          </v-btn>
        </v-card-actions>
      </v-card>
...
async function addCrop() {
  ...
  if (!valid) return
  loading.value = true
  api.post('add_crop', item).then(res => {

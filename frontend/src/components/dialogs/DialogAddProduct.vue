  ...
  <div>
    <v-dialog v-model="props.model" persistent width="800px">
      <v-card :loading="loading" min-height="">
            ...
            <v-col cols="11" class="mt-1">
              <v-row>
                ...
                <v-col cols="11" class="pl-0">
                  <span :style="'color:' + color">Adicionar Produto</span>
                </v-col>
              </v-row>
            </v-col>
            <v-col cols="1">
        ...
        <v-card-subtitle>
          <span v-if="loading">Inserindo produto</span>
          <span v-else>Defina o novo produto</span>
        </v-card-subtitle>
        ...
        <v-card-actions class="pa-3" v-if="!loading">
          ...
          <v-hover>
            <template v-slot:default="{ isHovering, props }">
              <v-btn v-bind="props" :color="color" :variant="isHovering ? 'tonal' : 'elevated'" @click="addProduct()">
                Confirmar
              </v-btn>
            </template>
          </v-hover>
...
async function addProduct() {
  ...
  loading.value = true
  api.post('add_product', item).then((response) => {

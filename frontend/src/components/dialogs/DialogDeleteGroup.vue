  ...
  <div>
    <v-dialog v-model="props.model" persistent width="800px">
      <v-card :loading="loading" min-height="">
            ...
            <v-col cols="11" class="mt-1">
              <v-row>
                ...
                <v-col cols="11" class="pl-0">
                  <span :style="'color:' + color">Deletar Grupo</span>
                </v-col>
              </v-row>
            </v-col>
            <v-col cols="1">
        ...
        <v-card-subtitle>
          <span v-if="loading">Excluindo grupo...</span>  
          ...
          <v-row align="center" justify="center" class="ma-1 mt-0">
            <v-col cols="12">
              <v-alert style="color: white" :color="color" border="start" elevation="3" dark icon="mdi-delete-alert">
                ...
                <div v-if="!loading">
                  <span style="font-size: 1.3vw;" class="text-shadow-black-1">
                    Os participantes do grupo deletado receberão o grupo "Padrão". 
                    <br> 
                    Realoque-os para o grupo correto o mais rápido possível.
                  </span> 
                  <br>
                  <span class="font-italic">Grupo: </span> {{ data.name }} 
                  <br>                  
          ...
          <v-hover>
            <template v-slot:default="{ isHovering, props }">
              <v-btn v-bind="props" :color="color"
                :variant="isHovering ? 'tonal' : 'elevated'" @click="deleteGroup()">
                <span>Confirmar</span>
              </v-btn>
            </template>
          </v-hover>
...
async function deleteGroup() {
  loading.value = true
  api.delete('delete_group/' + props.data.id).then((response) => {

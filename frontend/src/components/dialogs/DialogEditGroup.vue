  ...
  <div>
    <v-dialog v-model="props.model" persistent width="800px">
      <v-card :loading="loading" min-height="">
            ...
            <v-col cols="11" class="mt-1">
              <v-row>
                ...
                <v-col cols="11" class="pl-0">
                  <span :style="'color:' + color">Editar Grupo</span>
                </v-col>
              </v-row>
            </v-col>
            <v-col cols="1">
        ...
        <v-card-subtitle>
          <span v-if="loading">Editando grupo...</span>
          <span v-else>Quando editado, esse grupo afetará a dashboard.</span>
        </v-card-subtitle>
        ...
        <v-card-actions class="pa-3" v-if="!loading">
          ...
          <v-hover>
            <template v-slot:default="{ isHovering, props }">
              <v-btn v-bind="props" :color="color" :variant="isHovering ? 'tonal' : 'elevated'" @click="editGroup()">
                <span>Confirmar</span>
              </v-btn>
            </template>
          </v-hover>
...
async function editGroup() {
  ...
  loading.value = true
  api.put('edit_group/' + item.id, item).then((response) => {

  ...
  <div>
    <v-dialog v-model="props.model" persistent width="800px">
      <v-card :loading="loading" min-height="">
            ...
            <v-col cols="11" class="mt-1">
              <v-row>
                ...
                <v-col cols="11" class="pl-0">
                  <span :style="'color:' + color">Editar Produto</span>
                </v-col>
              </v-row>
            </v-col>
            <v-col cols="1">
        ...
        <v-card-subtitle>
          <span v-if="loading">Editando produto...</span>
          <span v-else>Quando editado, esse produto continua atribuído às campanhas que já o possuem.</span>
        </v-card-subtitle>
        ...
        <v-card-text class="mb-5" v-if="!loading">
          <v-form ref="form">
            <v-row>
              ...
              <v-col cols="12" md="3">
                <v-select label="Critério" v-model="item.type" :color="color"
                  :items="[{ title: 'Ambos', value: 0 }, { title: 'Valor', value: 1 }, { title: 'Quantidade', value: 2 }]"></v-select>
              </v-col>
            </v-row>
          </v-form>
        </v-card-text>
        ...
        <v-card-actions class="pa-3" v-if="!loading">
          ...
          <v-hover>
            <template v-slot:default="{ isHovering, props }">
              <v-btn v-bind="props" :color="color" :variant="isHovering ? 'tonal' : 'elevated'" @click="editProduct()">
                <span>Confirmar</span>
              </v-btn>
            </template>
          </v-hover>
...
async function editProduct() {
  ...
  loading.value = true
  api.put('edit_product/' + item.id, item).then((response) => {

<template>
    <div>
                    ...
                    <v-row>
                            ...
                            <v-hover>
                                <template v-slot:default="{ isHovering, props }">
                                    <div v-bind="props" class="align-center" style="position: relative">
                                        <v-avatar @click="my_profile && openDialogEditProfile()" size="17vw"
                                            style="transition: 0.2s;"
                                            :style="(my_profile ? 'cursor: pointer; ' : '') + (dark_theme ? (isHovering ? 'border: solid 4px rgb(255, 255, 255, 0.4);' : 'border: solid 2px rgb(255, 255, 255, 0.15);') : (isHovering ? 'border: solid 4px rgb(255, 255, 255);' : 'border: solid 2px rgb(255, 255, 255);'))">
                                            <v-img v-if="user_data.photo" style="transition: transform 0.3s"
                                                :style="isHovering ? 'transform: scale(1.1)' : ''"
                                                :src="backend_storage + user_data.photo" cover
                                            ...
                                            <v-img v-else src="media/images/man.png"
                                                style="transition: 0.4s ease-in-out"
                                                :style="'object-fit: cover; width: 64px;'">
                                            </v-img>
                                        </v-avatar>
                                        <v-tooltip text="Editar Perfil"
                                            :content-class="dark_theme ? 'tooltip-default-dark' : 'tooltip-default-light'">
                                            <template v-slot:activator="{ props }">
                                                <v-scale-transition>
                                                    <v-btn v-if="isHovering && my_profile" v-bind="props"
                                                        @click="openDialogEditProfile()" color="white" size="2vw"
                                                        icon="mdi-account-edit" class="account-edit-icon"></v-btn>
                                                </v-scale-transition>
                                            </template>
                                        </v-tooltip>
                                        <v-tooltip v-if="last_login" location="top" content-class="tooltip-green"
                                            :model-value="isHovering">
                                            <template v-slot:activator="{ props }">
                                                <v-icon v-bind="props" color="green" size="3vw"
                                                    class="last-login-icon">mdi-radiobox-marked</v-icon>
                                            </template>
                        ...
                        <v-col cols="12" class="align-center">
                            <span class="user-name"
                                :style="'text-shadow: 2px 1px ' + (dark_theme ? 'black' : 'white')">{{
                                    user_data?.full_name
                                ?? user_data.name }}</span>
                        </v-col>
                        <v-col cols="12" class="align-center">
                            <v-hover>
                                <template v-slot:default="{ isHovering, props }">
                                    <div v-bind="props" @click="my_profile ? edit_note = true : null"
                                        class="user-note align-center"
                                        :style="(isHovering && my_profile ? 'cursor: pointer;' : '') + ('width: 95%;') + (dark_theme ? '' : 'box-shadow: rgba(0, 0, 0, 0.07) 0px -23px 25px 0px inset, rgba(0, 0, 0, 0.05) 0px -36px 30px 0px inset, rgba(255, 255, 255, 0.4) 0px -79px 40px 0px inset, rgba(0, 0, 0, 0.06) 0px 0px 0px, rgba(0, 0, 0, 0.01) 0px 0px 0px, rgba(0, 0, 0, 0.02) 0px 0px 4px, rgba(0, 0, 0, 0.516) 0px 1px 2px;')">
                                        <v-tooltip text="Alterar status &#128526;" content-class="tooltip-blue"
                                            :model-value="isHovering && my_profile">
                                            <template v-slot:activator="{ props }">
                                                <v-icon size="2vw" v-bind="my_profile ? props : null"
                                                    class="user-note-icon"
                                                    :style="dark_theme ? 'text-shadow: rgb(0, 0, 0) 1.5px 1px;' : 'text-shadow: rgb(255, 255, 255) 1.5px 1px;'">mdi-comment-quote</v-icon>
                                            </template>
                                        </v-tooltip>
                                        <v-scale-transition>
                                            <v-btn v-if="isHovering && my_profile && !edit_note" size="2.2vw"
                                                icon="mdi-pencil-plus" color="blue"
                                                style="position: absolute; font-size: 1vw; z-index: 1;"></v-btn>
                                        </v-scale-transition>
                                        <span v-if="!edit_note" class="user-note-text"
                                            :style="isHovering && my_profile ? 'opacity: 0.2' : ''">{{
                                                user_data.note ? user_data.note : 'Defina um status...' }}</span>
                                        <v-textarea v-else ref="editNoteTextarea" color="blue" :loading="loading_note"
                                            :disabled="loading_note" @keydown.enter="saveNote()" @blur="saveNote()"
                                            style="width: 100%; transition: 0.3s;" v-model="user_data.note"
                                            placeholder="Qual o status de hoje?" clearable counter maxlength="200"
                                            auto-grow rows="1">
                                        </v-textarea>
                                    </div>
                                </template>
                            ...
                            <v-card class="about-card" :loading="loading_user">
                                <v-list class="about-card-list">
                                    <v-list-subheader>
                                        ...
                                        <v-btn v-if="my_profile" @click="openDialogEditProfile()"
                                            prepend-icon="mdi-account-edit"
                                            style="height: 1.8vw; font-size: 0.9vw; position: absolute; top: 8px; right: 8px;"
                                            color="blue">Editar</v-btn>
                                    </v-list-subheader>
                                    <v-list-item class="about-card-list-item"
                                        :class="dark_theme ? 'text-shadow-black-1' : ''">
                                        <template v-slot:prepend>
                                            <v-icon icon="mdi-email"></v-icon>
                                        </template>
                                        <v-list-item-title>
                                            <span v-if="!loading_user" class="about-card-list-info">{{ user_data.email
                                                }}</span>
                                        </v-list-item-title>
                                    </v-list-item>
                                    <v-list-item class="about-card-list-item"
                                        ...
                                        <template v-slot:prepend>
                                            <v-icon
                                                :icon="user_data.level > 0 ? 'mdi-shield-lock-open' : 'mdi-shield-lock'"></v-icon>
                                        </template>
                                        <v-list-item-title>
                                            <span v-if="!loading_user" class="about-card-list-info">{{ 'Acesso ' +
                                                accesses_translations[user_data.level] }}</span>
                                        </v-list-item-title>
                                    </v-list-item>
                                    <v-list-item class="about-card-list-item"
                                        :class="dark_theme ? 'text-shadow-black-1' : ''">
                                        <template v-slot:prepend>
                                            <v-icon
                                                :icon="user_data.status > 0 || my_profile ? 'mdi-account-check' : 'mdi-account-cancel'"></v-icon>
                                        </template>
                                        <v-list-item-title>
                                            <span v-if="!loading_user" class="about-card-list-info">{{ 'Usuário ' +
                                                (user_data.status > 0 ||
                                                my_profile ? 'Ativo' : 'Desativado') }}</span>
                                        </v-list-item-title>
                                    </v-list-item>
                                </v-list>
                            </v-card>
        ...
        <DialogEditProfile @close="closeDialogEditProfile()" :model="profile_dialog" :data="profile_form"
            color="orange" />
    </div>
</template>
<script setup>

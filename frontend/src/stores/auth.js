...
export const useAuthStore = defineStore('auth', () => {
    const token = ref(localStorage.getItem('agriflow_token'))
    const user = ref(JSON.parse(localStorage.getItem('agriflow_user')))    
    const is_auth = ref(false)
    const router = useRouter()

    function setToken(token_value) {
        localStorage.setItem('agriflow_token', token_value)
        token.value = token_value
    }

    function setUser(user_value) {
        localStorage.setItem('agriflow_user', JSON.stringify(user_value))
        user.value = user_value
    }

    function setUserAttributes(attributes) {
        Object.assign(user.value, attributes)
        localStorage.setItem('agriflow_user', JSON.stringify(user.value))
    }
    ...
    function hasData() {
        return (token.value != null && user.value != null && localStorage.getItem('agriflow_token') != null && localStorage.getItem('agriflow_user') != null)
    }
